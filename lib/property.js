...
Property.prototype = {
  ...
  toString: function( version ) {
    ...
    var propName = (this.group ? this.group + '.' : '') + dashCase( this._field )
    var keys = Object.keys( this )
    var params = []

    for( var i = 0; i < keys.length; i++ ) {
      if (keys[i] === 'group') continue
      params.push( dashCase( keys[i] ) + '=' + this[ keys[i] ] )
    }
